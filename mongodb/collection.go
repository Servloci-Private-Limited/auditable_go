...
type Config struct {
	...
	// ChangeComputer replaces the built-in BSON diff algorithm. It is called
	// without library-held locks and receives a deadline-aware context.
	ChangeComputer ChangeComputer

	// DeferredChanges handles requests that exceed DiffLimits. Implementations
	// commonly store snapshots in object storage and enqueue delayed computation.
	DeferredChanges DeferredChangeHandler

	DiffLimits DiffLimits

	// FailOnAuditError returns audit/diff errors from mutation methods. The
	// business mutation may already be committed unless the caller uses a MongoDB
	// transaction, so callers must still reconcile returned errors.
	FailOnAuditError bool

	// VersionCollectionName overrides the atomic sequence collection.
...
func (a *AuditableCollection) fieldPolicy(extra structAuditInfo) FieldPolicy {
	...
	return FieldPolicy{
		SkipFields:     cloneStringSet(mergeStringSets(mergeStringSets(a.config.SkipFields, a.auditInfo.skip), extra.skip)),
		RedactedFields: cloneStringSet(mergeStringSets(mergeStringSets(a.config.RedactedFields, a.auditInfo.redacted), extra.redacted)),
		OnlyFields:     cloneStringSet(only),
		RedactedValue:  a.config.RedactedValue,
	}
}
